    ...
    return (
        <Box sx={{ maxWidth: 700, mx: 'auto', mt: 4 }}>
            <Paper sx={{ p: 3 }}>
                ...
                <Typography variant="body2" color="text.secondary" gutterBottom>
                    Fecha: {new Date(log.timestamp).toLocaleString()} | Usuario: {log.performedByEmail || '—'}
                </Typography>
                {log.restoredAt && (
                ...
                {log.action === 'update' && auditData.previous && auditData.current ? (
                    <Box>
                        ...
                        <Box sx={{ display: 'flex', gap: 2 }}>
                            <Box sx={{ flex: 1 }}>
                                ...
                                <pre style={{ background: '#f5f5f5', padding: 8, borderRadius: 4 }}>{JSON.stringify(JSON.parse(auditData.previous), null, 2)}</pre>
                            </Box>
                            <Box sx={{ flex: 1 }}>
                                <Typography variant="subtitle2">Actual</Typography>
                                <pre style={{ background: '#f5f5f5', padding: 8, borderRadius: 4 }}>{JSON.stringify(JSON.parse(auditData.current), null, 2)}</pre>
                            </Box>
                        </Box>
                    </Box>
                ) : (
                    <Box>
                        ...
                        <pre style={{ background: '#f5f5f5', padding: 8, borderRadius: 4 }}>{log.data}</pre>
                    </Box>
                )}
